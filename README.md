...
设备回归可运行 `tools/verify-device.ps1`，会重开测试对局，默认保留排行榜；只有显式 `-ResetData` 才会清除应用数据。回归读取无障碍文本，不截图，报告与临时 XML 均放在 `E:\temp\TVMinesweeper`。实际验证结果以验证记录和脚本报告为准。
